...
class Contract:
    ...
    def discount_rates(self, market_rates):
        discount_rates = np.ones(len(market_rates) + 1)
        for i in range(1,len(market_rates) + 1):
            discount_rates[i] = 1 / (1 + market_rates[i]) * discount_rates[i - 1]
    ...
    def calculate_npvs(self, market_rates): # LEEVI, DO THIS!!!!!
        ...
        discount_rates = self.discount_rates(market_rates)
